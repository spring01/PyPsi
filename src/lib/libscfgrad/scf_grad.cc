...
SCFGrad::SCFGrad(Process::Environment& process_environment_in) :
    Wavefunction(process_environment_in, process_environment_in.options, process_environment_in.psio()) 
{
    common_init();
}
SCFGrad::~SCFGrad()
...
SharedMatrix SCFGrad::compute_gradient()
{
    ...
    if (options_.get_str("REFERENCE") == "RKS") {
        potential = VBase::build_V(process_environment_, options_, "RV"); 
        ...
        potential = VBase::build_V(process_environment_, options_, "UV"); 
        potential->initialize();
        std::vector<SharedMatrix>& C = potential->C();
        C.push_back(Ca_subset("SO", "OCC"));
    ...
    boost::shared_ptr<JKGrad> jk = JKGrad::build_JKGrad(process_environment_, 1);
