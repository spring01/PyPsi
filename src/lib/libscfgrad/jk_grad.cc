...
JKGrad::JKGrad(int deriv, boost::shared_ptr<BasisSet> primary, Process::Environment& process_environment_in) :
    deriv_(deriv), primary_(primary), process_environment_(process_environment_in)
{
    common_init();
}
JKGrad::~JKGrad()
...
boost::shared_ptr<JKGrad> JKGrad::build_JKGrad(Process::Environment& process_environment_in, int deriv)
{
    ...
    if (options.get_str("SCF_TYPE") == "DF") {
        ...
        DFJKGrad* jk = new DFJKGrad(deriv,primary,auxiliary, process_environment_in);
        ...
        DirectJKGrad* jk = new DirectJKGrad(deriv,primary, process_environment_in);
...
DFJKGrad::DFJKGrad(int deriv, boost::shared_ptr<BasisSet> primary, boost::shared_ptr<BasisSet> auxiliary, Process::Environment& process_environment_in) :
    JKGrad(deriv,primary, process_environment_in), auxiliary_(auxiliary)
{
    common_init();
}
DFJKGrad::~DFJKGrad()
...
void DFJKGrad::common_init()
{
    ...
    psio_ = process_environment_.psio();
}
...
DirectJKGrad::DirectJKGrad(int deriv, boost::shared_ptr<BasisSet> primary, Process::Environment& process_environment_in) :
    JKGrad(deriv,primary, process_environment_in)
{
    common_init();
}
