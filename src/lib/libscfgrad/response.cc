...
boost::shared_ptr<Matrix> SCFGrad::rhf_hessian_response()
{
    ...
    boost::shared_ptr<JK> jk = JK::build_JK(process_environment_, process_environment_.psio());
    ...
    // => CPHF (Uai) <= //
    {
        boost::shared_ptr<RCPHF> cphf(new RCPHF(process_environment_, process_environment_.psio()));
        cphf->set_jk(jk);
