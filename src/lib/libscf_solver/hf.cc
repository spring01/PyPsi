...
void HF::guess()
{
    ...
    if (guess_type == "ORBITAL" && guessOrbital_.size() != 2) {
        fprintf(outfile, "  SCF Guess was ORBITAL but guessOrbital_ is empty.\n");
    ...
    if (guess_type == "READ") {
        ...
        find_occupation();
        Ca_->copy(guessOrbital_[0]);
        Cb_->copy(guessOrbital_[1]);
        form_D();
